...
async def create_abonnement(
    ...
    if pme:
        ...
        confirmation_link = f"http://localhost:8000/confirm-abonnement/{new_abonnement.id}/{uuid4()}"
    
        # Envoi de l'email de confirmation en tâche de fond
        email_subject = "Demande de confirmation d'abonnement"
        email_body = f"""
        Bonjour {pme.utilisateur.nom_prenom},

        Vous avez reçu une nouvelle demande d'abonnement de la part de {current_user.nom_prenom}.
        Numéro d'abonnement : {new_abonnement.num_abonnement}

        Cliquez sur le lien ci-dessous pour valider l'abonnement :
        {confirmation_link}

        Cordialement,
        Votre équipe de gestion des abonnements
        """
        
        # Envoi de l'email en tâche de fond pour ne pas bloquer le processus
        BackgroundTasks.add_task(send_confirmation_email, pme.utilisateur.email, email_subject, email_body)
